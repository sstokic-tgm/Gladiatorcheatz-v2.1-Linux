...
#define NETVAR(type, name, table, netvar)                           \
    type& name() const {                                          \
        static int _name = NetMngr::Get().getOffs(table, netvar);     \
        return *(type*)((uintptr_t)this + _name);                 \
	}

#define NETVARADDOFFS(type, name, table, netvar, offs)                           \
    type& name() const {                                          \
        static int _name = NetMngr::Get().getOffs(table, netvar) + offs;     \
        return *(type*)((uintptr_t)this + _name);                 \
	}
...
public:
    ...
    static inline C_BaseEntity* GetEntityByIndex(int index)
    {
        return static_cast<C_BaseEntity*>(g_EntityList->GetClientEntity(index));
    }
    static inline C_BaseEntity* get_entity_from_handle(CBaseHandle h)
    {
    ...
    {
        typedef datamap_t*(*o_GetPredDescMap)(void*);
    ...
    {
        vfunc<void(*)(C_BaseEntity*, int)>(this, 75)(this, index);
    }
...
public:
    ...
    static inline C_BasePlayer* GetPlayerByUserId(int id)
    {
        return static_cast<C_BasePlayer*>(GetEntityByIndex(g_EngineClient->GetPlayerForUserID(id)));
    }

    static inline C_BasePlayer* GetPlayerByIndex(int i)
    {
        return static_cast<C_BasePlayer*>(GetEntityByIndex(i));
    }

    static inline C_BasePlayer *GetLocalPlayer(const bool set = false)
    {
...
public:
    ...
    __int32_t						m_iReapplyProvisionParity; //0x0018
    uintptr_t						m_hOuter; //0x001C
    char	_pad_0x0020[0x4];
    __int32_t						m_ProviderType; //0x0024
