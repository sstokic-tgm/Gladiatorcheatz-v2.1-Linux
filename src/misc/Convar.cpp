...
void ConVar_Register(int nCVarFlag, IConCommandBaseAccessor* pAccessor) {
  if (!g_CVar || s_bRegistered) return;
...
const char* CCommand::FindArg(const char* pName) const {
  ...
  for (int i = 1; i < nArgC; i++) {
    if (!strcasecmp(Arg(i), pName)) return (i + 1) < nArgC ? Arg(i + 1) : "";
  }
  return 0;
}
...
int DefaultCompletionFunc(
    const char* partial,
    char
        commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH]) {
  return 0;
}
...
int ConVar::GetFlags(void) const { return m_pParent->m_nFlags; }
...
int ConVar::GetSplitScreenPlayerSlot(void) const { return 0; }
...
void ConVar::ChangeStringValue(const char* tempVal, float flOldValue) {
  ...
  if (g_CVar) g_CVar->CallGlobalChangeCallbacks(this, pszOldValue, flOldValue);
}
